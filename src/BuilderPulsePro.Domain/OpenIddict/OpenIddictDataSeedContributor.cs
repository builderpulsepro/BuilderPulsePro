...
namespace BuilderPulsePro.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("BuilderPulsePro") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "BuilderPulsePro", DisplayName = "BuilderPulsePro API", Resources = { "BuilderPulsePro" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var consoleAndAngularClientId = configurationSection["BuilderPulsePro_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["BuilderPulsePro_App:RootUrl"]?.TrimEnd('/');
        ...
        //MAUI Client
        var mauiClientId = configurationSection["BuilderPulsePro_Maui:ClientId"];
        if (!mauiClientId.IsNullOrWhiteSpace())
        {
            var mauiClientRootUrl = configurationSection["BuilderPulsePro_Maui:RootUrl"]?.Replace("_", "-");
            var mauiClientPostLogoutRedirectUri = configurationSection["BuilderPulsePro_Maui:PostLogoutRedirectUri"]?.Replace("_", "-");
            await CreateApplicationAsync(
                name: mauiClientId!,
                type: OpenIddictConstants.ClientTypes.Public,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
        ...
        var blazorServerClientId = configurationSection["BuilderPulsePro_BlazorServer:ClientId"];
        if (!blazorServerClientId.IsNullOrWhiteSpace())
        {
            var blazorServerRootUrl = configurationSection["BuilderPulsePro_BlazorServer:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["BuilderPulsePro_BlazorServer:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["BuilderPulsePro_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["BuilderPulsePro_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        var webPublicTieredClientId = configurationSection["BuilderPulsePro_Web_Public:ClientId"];
        if (!webPublicTieredClientId.IsNullOrWhiteSpace())
        {
            var webPublicTieredRootUrl = configurationSection["BuilderPulsePro_Web_Public:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["BuilderPulsePro_Web_Public:ClientSecret"] ?? "1q2w3e*",
                grantTypes: new List<string> //Hybrid flow
                {
                    OpenIddictConstants.GrantTypes.AuthorizationCode, OpenIddictConstants.GrantTypes.Implicit
                },
