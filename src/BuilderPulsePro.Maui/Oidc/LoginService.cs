...
using BuilderPulsePro.Maui.Storage;
...
public class LoginService : ILoginService, ITransientDependency
{
    ...
    {
        var token = await _storage.GetAsync(BuilderPulseProConsts.OidcConsts.AccessTokenKeyName);
    ...
    {
        var refreshToken = await _storage.GetAsync(BuilderPulseProConsts.OidcConsts.RefreshTokenKeyName);
    ...
    {
        await _storage.SetAsync(BuilderPulseProConsts.OidcConsts.AccessTokenKeyName, accessToken);
        await _storage.SetAsync(BuilderPulseProConsts.OidcConsts.RefreshTokenKeyName, refreshToken);
    }

    private async Task ClearTokenCacheAsync()
    {
        await _storage.RemoveAsync(BuilderPulseProConsts.OidcConsts.AccessTokenKeyName);
        await _storage.RemoveAsync(BuilderPulseProConsts.OidcConsts.RefreshTokenKeyName);
    }
}